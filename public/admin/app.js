var B=Object.defineProperty;var N=(t,e,s)=>e in t?B(t,e,{enumerable:!0,configurable:!0,writable:!0,value:s}):t[e]=s;var h=(t,e,s)=>N(t,typeof e!="symbol"?e+"":e,s);function o(t){let e=document.createElement("div");return e.textContent=t,e.innerHTML}function b(t){if(t==null||t==="")return null;let e=Number(t);return!Number.isFinite(e)||!Number.isInteger(e)||e<=0?null:e}function r(t,e=document){return e.querySelector(t)}function f(t,e=document){return Array.from(e.querySelectorAll(t))}function l(t,e="success"){let s=r("#toast-container");if(!s)return;let n=document.createElement("div");n.className=`toast ${e}`,n.textContent=t,s.appendChild(n),setTimeout(()=>{n.style.animation="slideIn 0.3s ease reverse",setTimeout(()=>n.remove(),300)},3e3)}function y(t,e=2){if(t===0)return"0 B";let s=1024,n=e<0?0:e,a=["B","KB","MB","GB","TB"],i=Math.floor(Math.log(t)/Math.log(s));return`${parseFloat((t/Math.pow(s,i)).toFixed(n))} ${a[i]}`}function M(t){let e=Date.now(),s=typeof t=="object"?t.getTime():new Date(t).getTime(),n=e-s,a=Math.floor(n/1e3),i=Math.floor(a/60),d=Math.floor(i/60),m=Math.floor(d/24);return m>0?`${m} \u5929\u524D`:d>0?`${d} \u5C0F\u65F6\u524D`:i>0?`${i} \u5206\u949F\u524D`:"\u521A\u521A"}function w(t){let e=Math.floor(t/3600),s=Math.floor(t%3600/60),n=Math.floor(t%60);return e>0?`${e}:${s.toString().padStart(2,"0")}:${n.toString().padStart(2,"0")}`:`${s}:${n.toString().padStart(2,"0")}`}var x=class{constructor(){h(this,"ws",null);h(this,"handlers",new Map);h(this,"reconnectAttempts",0);h(this,"maxReconnectAttempts",5);h(this,"reconnectDelay",1e3);h(this,"url");let e=location.protocol==="https:"?"wss:":"ws:";this.url=`${e}//${location.host}/ws`}connect(){this.ws?.readyState!==WebSocket.OPEN&&(this.ws=new WebSocket(this.url),this.ws.addEventListener("open",()=>{this.reconnectAttempts=0,console.debug("[ws] connected")}),this.ws.addEventListener("message",e=>{try{let s=JSON.parse(e.data);this.dispatch(s.type,s.data)}catch(s){console.error("[ws] failed to parse message:",s)}}),this.ws.addEventListener("close",()=>{console.debug("[ws] disconnected"),this.scheduleReconnect()}),this.ws.addEventListener("error",e=>{console.error("[ws] error:",e)}))}disconnect(){this.maxReconnectAttempts=0,this.ws?.close(),this.ws=null}on(e,s){return this.handlers.has(e)||this.handlers.set(e,new Set),this.handlers.get(e).add(s),()=>{this.handlers.get(e)?.delete(s)}}off(e,s){this.handlers.get(e)?.delete(s)}dispatch(e,s){this.handlers.get(e)?.forEach(n=>{try{n(s)}catch(a){console.error(`[ws] handler error for ${e}:`,a)}})}scheduleReconnect(){if(this.reconnectAttempts>=this.maxReconnectAttempts){console.warn("[ws] max reconnect attempts reached");return}let e=this.reconnectDelay*Math.pow(2,this.reconnectAttempts);this.reconnectAttempts++,console.debug(`[ws] reconnecting in ${e}ms (attempt ${this.reconnectAttempts})`),setTimeout(()=>this.connect(),e)}},g=new x;var p="";function F(t){return t.ok?t.json():Promise.reject(new Error(`HTTP ${t.status}: ${t.statusText}`))}async function u(t,e){let s=await fetch(t,e);return F(s)}var c={status:()=>u(`${p}/api/status`),ffmpegStatus:()=>u(`${p}/api/ffmpeg/status`),triggerFfmpegDownload:()=>u(`${p}/api/ffmpeg/download`,{method:"POST"}),sourceStart:(t,e)=>u(`${p}/api/source/start`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:t,id:e})}),sourceStop:()=>u(`${p}/api/source/stop`,{method:"POST"}),upload:async t=>{let e=new FormData;e.append("file",t);let s=await fetch(`${p}/api/source/upload`,{method:"POST",body:e});return F(s)},listFiles:()=>u(`${p}/api/source/files`),deleteFile:t=>u(`${p}/api/source/files/${t}`,{method:"DELETE"}),listPlaylist:()=>u(`${p}/api/playlist`),addToPlaylist:(t,e,s)=>u(`${p}/api/playlist`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({filename:t,displayName:e,durationSec:s})}),deleteFromPlaylist:t=>u(`${p}/api/playlist/${t}`,{method:"DELETE"}),reorderPlaylist:t=>u(`${p}/api/playlist/reorder`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ids:t})}),listArchive:()=>u(`${p}/api/archive/list`),currentListeners:()=>u(`${p}/api/listeners/current`),historyListeners:(t=1)=>u(`${p}/api/listeners/history?page=${t}`),config:()=>u(`${p}/api/config`),updateConfig:(t,e)=>u(`${p}/api/config`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({key:t,value:e})})};function H(t){let e=new EventSource(`${p}/api/ffmpeg/download/status`);return e.onmessage=s=>{try{t(JSON.parse(s.data))}catch{}},()=>e.close()}function P(){let t=r("#dashboard-view");t&&(j(t),L(),U())}function j(t){t.innerHTML=`
  ...
  `,f(".copy-btn").forEach(e=>{e.addEventListener("click",()=>{let s=e.getAttribute("data-copy");s&&(navigator.clipboard.writeText(location.origin+s),l("\u5DF2\u590D\u5236\u5230\u526A\u8D34\u677F","success"))})})}async function L(){try{let[t,e]=await Promise.all([c.status(),c.ffmpegStatus().catch(()=>null)]);W(t),_(e)}catch(t){console.error("[dashboard] failed to load:",t)}}function W(t){let e=r("#stat-live-text"),s=r("#stat-live"),n=r("#stat-listeners"),a=r("#stat-bitrate"),i=r("#segment-list");if(e&&s){let d=!!t.broadcaster?.isLive;e.textContent=d?"LIVE":"OFFLINE",s.className=`stat-value ${d?"text-success":"text-muted"}`}n&&(n.textContent=String(t.listeners?.count??0)),a&&(a.textContent="--"),i&&(i.innerHTML='<p class="text-muted">\u6682\u65E0\u5207\u7247</p>')}function _(t){let e=r("#ffmpeg-status");if(e){if(!t||!t.available){e.innerHTML='<p class="text-muted">FFmpeg \u672A\u5B89\u88C5\u6216\u65E0\u6CD5\u83B7\u53D6\u72B6\u6001</p>';return}e.innerHTML=`
    <table>
      <tr>
  ...
  `}}function U(){g.on("source-start",()=>{L()}),g.on("source-end",()=>{L()}),g.on("listener-count",()=>{L()}),g.on("archive-new",()=>{L()})}async function k(t){t.innerHTML=`
    <div class="card">
  ...
  `;let e=1;r("#prev-page")?.addEventListener("click",async()=>{e>1&&(e--,await S(e))}),r("#next-page")?.addEventListener("click",async()=>{e++,await S(e)}),await Promise.all([q(),S(e)])}async function q(){let t=r("#current-listeners");if(t)try{let s=(await c.currentListeners())?.listeners??[];if(s.length===0){t.innerHTML='<p class="text-muted">\u5F53\u524D\u65E0\u542C\u4F17\u5728\u7EBF</p>';return}t.innerHTML=`
    ...
    `,s&&(s.textContent=`\u7B2C ${t} / ${T} \u9875`),n&&(n.disabled=t<=1),a&&(a.disabled=t>=T)}catch(i){e.innerHTML='<p class="text-muted">\u52A0\u8F7D\u5931\u8D25</p>',console.error("[listeners] history load error:",i)}}async function R(t){t.innerHTML=`
    <div class="card">
  ...
  `,await J()}async function J(){let t=r("#archive-list");if(t)try{let s=(await c.listArchive())?.files??[];if(s.length===0){t.innerHTML='<p class="text-muted">\u6682\u65E0\u5F55\u5236\u56DE\u653E</p>';return}let n={};for(let i of s){let d=new Date(i.mtime).toLocaleDateString("zh-CN",{year:"numeric",month:"2-digit",day:"2-digit"});n[d]||(n[d]=[]),n[d].push(i)}let a="";for(let[i,d]of Object.entries(n))a+=`
      ...
      `;t.innerHTML=a}catch(e){t.innerHTML='<p class="text-muted">\u52A0\u8F7D\u5931\u8D25</p>',console.error("[archive] load error:",e)}}async function D(t){t.innerHTML=`
    <div class="card">
      <div class="card-title">FFmpeg \u72B6\u6001</div>
  ...
  `,await I()}async function I(){let t=r("#ffmpeg-status-content"),e=r("#ffmpeg-download-content");try{let s=await c.ffmpegStatus();t&&(t.innerHTML=`
        <table>
          <tr>
      ...
      `),e&&(s.available?e.innerHTML=`
        ...
        `:(e.innerHTML=`
          ...
          ${K()?'<p class="text-muted" style="margin-top: 1rem"><strong>\u63D0\u793A\uFF1A</strong> macOS \u53EF\u80FD\u9700\u8981\u5141\u8BB8"\u4EFB\u4F55\u6765\u6E90"\u5E94\u7528\u4EE5\u8FD0\u884C FFmpeg\u3002\u8BF7\u5728\u7EC8\u7AEF\u8FD0\u884C\uFF1A<code>sudo spctl --master-disable</code></p>':""}
        `,r("#download-ffmpeg-btn")?.addEventListener("click",V)))}catch(s){console.error("[ffmpeg-panel] status error:",s),t&&(t.innerHTML='<p class="text-muted">\u65E0\u6CD5\u83B7\u53D6 FFmpeg \u72B6\u6001</p>'),e&&(e.innerHTML='<p class="text-muted">\u52A0\u8F7D\u5931\u8D25</p>')}}async function V(){let t=r("#download-progress"),e=r("#download-ffmpeg-btn");if(!t)return;e&&(e.disabled=!0,e.textContent="\u4E0B\u8F7D\u4E2D...");try{await c.triggerFfmpegDownload()}catch(n){console.error("[ffmpeg-panel] trigger error:",n)}let s=H(n=>{if(t)switch(n.state){case"downloading":t.innerHTML=`
        ...
        `;break;case"complete":t.innerHTML='<p class="text-success">\u2713 FFmpeg \u5B89\u88C5\u6210\u529F\uFF01</p>',setTimeout(()=>I(),1500);break;case"error":t.innerHTML=`<p class="text-error">\u2717 \u5B89\u88C5\u5931\u8D25: ${o(n.message)}</p>`,e&&(e.disabled=!1,e.textContent="\u91CD\u8BD5");break;case"idle":t.innerHTML='<p class="text-muted">\u7A7A\u95F2</p>';break}});setTimeout(()=>{t.innerHTML.includes("\u4E0B\u8F7D\u4E2D")&&s()},3e5)}function K(){return navigator.platform.toLowerCase().includes("mac")}function G(){let t=f(".tab-button");t.forEach(e=>{e.addEventListener("click",()=>{let s=e.getAttribute("data-tab");t.forEach(i=>i.classList.remove("active")),e.classList.add("active"),f(".tab-panel").forEach(i=>i.classList.remove("active"));let a=r(`#${s}-view`);a&&a.classList.add("active"),O(s)})})}function Z(){let t=r("#source-view");t&&k(t)}function Q(){let t=r("#listeners-view");t&&C(t)}function X(){let t=r("#archive-view");t&&R(t)}function Y(){let t=r("#ffmpeg-view");t&&D(t)}async function O(t){switch(t){case"dashboard":P();break;case"source":await Z();break;case"listeners":await Q();break;case"archive":await X();break;case"ffmpeg":await Y();break}}function tt(){G(),O("dashboard"),g.connect(),setInterval(async()=>{try{let e=await c.status();et(e)}catch(e){console.debug("[polling] failed:",e)}},5e3)}function et(t){let e=r("#disconnect-warning");e&&(!!t.broadcaster?.isLive?e.style.display="none":e.style.display="inline-block")}document.addEventListener("DOMContentLoaded",tt);
